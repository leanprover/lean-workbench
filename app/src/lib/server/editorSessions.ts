...
`
  ...
  private async buildOverlayArgs(
    ...
    const db = getDb()
    const packageSets = await db.projectPackageSet.findMany({
      where: { projectId },
      select: { packageSet: true },
    })
    const args: string[] = []
    for (const { packageSet: setName } of packageSets) {
    ...
    if (this.config.sandboxMode === 'bubblewrap') {
      ...
      child = spawn(
        ...
        [
          ...
          '/etc',
          '--ro-bind',
          this.config.openVscodeServerDir,
          '/workspace/.openvscode-server',
          '--ro-bind',
          this.config.elanDir,
          '/workspace/.elan',
          '--ro-bind',
          this.config.vscodeExtensionsDir,
          ...
          '--setenv',
          'PATH',
          `/workspace/.elan/bin:/usr/local/bin:/usr/bin:/bin`,
          '--setenv',
          'GIT_CONFIG_COUNT',
          '1',
          '--setenv',
...
let instance: EditorSessionManager | null = null

/** Return the singleton EditorSessionManager, creating it on first call. */
export function getEditorSessionManager(): EditorSessionManager {
  if (instance) return instance
  const cfg = getConfig()
  const dataDir = cfg.dataDir
  instance = new EditorSessionManager({
    workspacesDir: path.join(dataDir, 'workspaces'),
    elanDir: path.join(dataDir, 'elan'),
    openVscodeServerDir: env.OPENVSCODE_SERVER_DIR ?? '/app/.openvscode-server',
    vscodeExtensionsDir: env.VSCODE_EXTENSIONS_DIR ?? '/app/.vscode-extensions',
    nginxConfDir: env.NGINX_CONF_DIR ?? '/etc/nginx',
    nginxLogDir: env.NGINX_LOG_DIR ?? '/var/log/nginx',
    packageSetsDir: path.join(dataDir, 'package-sets'),
    sandboxMode: env.SANDBOX_MODE === 'off' ? 'off' : 'bubblewrap',
  })
  return instance
}
