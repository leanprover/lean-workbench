...
- Removed `is_first_run_complete` table. Use an `is_first_run_complete` field in `config.toml`.
- Changed `User.id` from `INTEGER AUTOINCREMENT` to a `String` UUID.
- Removed `auth_methods`. Store session secret in memory, OAuth client secret in `config.toml` (TODO reconsider).