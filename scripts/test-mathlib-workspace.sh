...
Options:
  ...
  --workspace PATH    Path to the workspace directory (relative to ROOT)
                      (default: auto-detect most recent under workspaces/)
...
# Auto-detect workspace (most recently modified)
if [ -z "$WORKSPACE_REL" ]; then
  WORKSPACE_ABS=$(find "$ROOT/workspaces" -name "lean-toolchain" -printf '%T@ %h\n' 2>/dev/null \
    | sort -rn | head -1 | cut -d' ' -f2-)
  if [ -z "$WORKSPACE_ABS" ]; then
    echo "Error: No workspace found under $ROOT/workspaces/"
    echo "Run mk-mathlib-workspace.sh first."
    exit 1
  fi
  # Convert absolute path to relative-to-ROOT
  WORKSPACE_REL="${WORKSPACE_ABS#$ROOT/}"
  echo "[test] Auto-detected workspace: $WORKSPACE_REL"
else
